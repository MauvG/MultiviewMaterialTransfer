...
class SevaParams(object):
    ...
    dropout: float = 0.0
    unflatten_names: list[str] = field(default_factory=lambda: ["middle_ds8", "output_ds4", "output_ds2"])
...
class Seva(nn.Module):
    def __init__(self, params: SevaParams) -> None:
        ...
        self.input_blocks = nn.ModuleList(
            [TimestepEmbedSequential(nn.Conv2d(params.in_channels, params.model_channels, 3, padding=1))]
        )
        ...
        for level, mult in enumerate(params.channel_mult):
            ...
            if level != len(params.channel_mult) - 1:
                ds *= 2
                out_ch = ch
                self.input_blocks.append(TimestepEmbedSequential(Downsample(ch, out_channels=out_ch)))
                ch = out_ch
                input_block_chans.append(ch)
                self._feature_size += ch
    ...
    def forward(
        ...
        for module in self.input_blocks:
            h = torch.utils.checkpoint.checkpoint(
                module, h, t_emb, y, dense_y, num_frames, sigma_val, use_reentrant=False
            )

            hs.append(h)

        h = torch.utils.checkpoint.checkpoint(
            self.middle_block, h, t_emb, y, dense_y, num_frames, sigma_val, use_reentrant=False
        )
...
class SGMWrapper(nn.Module):
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor, c: dict, sigma_val: float, **kwargs) -> torch.Tensor:
