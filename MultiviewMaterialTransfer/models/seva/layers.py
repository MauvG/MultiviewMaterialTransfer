    ...
    def forward(  # type: ignore[override]
        ...
        for layer in self:
            if isinstance(layer, MultiviewTransformer):
                ...
                x = layer(x, context, num_frames, sigma_val=sigma_val)
...
class ResBlock(nn.Module):
    def __init__(
        ...
        self.dense_emb_layers = nn.Sequential(nn.Conv2d(dense_in_channels, 2 * channels, 1, 1, 0))
    ...
    def forward(self, x: torch.Tensor, emb: torch.Tensor, dense_emb: torch.Tensor, sigma_val: float | None = None) -> torch.Tensor:
        return self.skip_connection(x) + torch.utils.checkpoint.checkpoint(
            self._forward, x, emb, dense_emb, use_reentrant=False
        )

    def _forward(self, x: torch.Tensor, emb: torch.Tensor, dense_emb: torch.Tensor) -> torch.Tensor:
