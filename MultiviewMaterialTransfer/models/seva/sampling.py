...
def make_betas(num_timesteps: int, linear_start: float = 1e-4, linear_end: float = 2e-2) -> np.ndarray:
...
class DDPMDiscretization(Discretization):
    ...
    def get_sigmas(self, n: int, device: str | torch.device = "cpu") -> torch.Tensor:
        ...
        return torch.flip(torch.tensor(sigmas, dtype=torch.float32, device=device), (0,))
...
class DiscreteDenoiser(object):
    ...
    def scaling(self, sigma: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        c_skip = torch.ones_like(sigma, device=sigma.device, dtype=sigma.dtype)
        c_out = -sigma
    ...
    def __call__(
        ...
        **additional_model_inputs,
        ...
        c_in = c_in.to(device=input.device, dtype=input.dtype)
        c_out = c_out.to(device=input.device, dtype=input.dtype)
        c_skip = c_skip.to(device=input.device, dtype=input.dtype)

        if debug:
            return (
                network(input * c_in, c_noise, cond, sigma_val=sigma_val, **additional_model_inputs) * c_out
                + input * c_skip,
                input.clone().detach().cpu(),
            )
...
class VanillaCFG(object):
    ...
    def _expand_scale(self, sigma: float | torch.Tensor, scale: float | torch.Tensor) -> float | torch.Tensor:
    ...
    def guidance(
        ...
        # Main stream uncond only
        results = uncond + scale * (cond - uncond)

        # results[21:42] = cond[21:42]

        return results

    def __call__(self, x: torch.Tensor, sigma: float | torch.Tensor, scale: float | torch.Tensor) -> torch.Tensor:
        x_u, x_c = x.chunk(2)
...
class MultiviewTemporalCFG(MultiviewCFG):
    def __init__(self, num_frames: int, cfg_min: float = 1.0):
        ...
        distance_matrix = (torch.arange(num_frames)[None] - torch.arange(num_frames)[:, None]).abs()
    ...
    def __call__(
        ...
        input_frame_mask = rearrange(input_frame_mask, "(b t) ... -> b t ...", t=self.num_frames)
        min_distance = (self.distance_matrix[None].to(x.device) + (~input_frame_mask[:, None]) * self.num_frames).min(
            -1
        )[0]
